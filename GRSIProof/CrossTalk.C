...
void CrossTalk::CreateHistograms() {
	...
	for(int det_num=1; det_num<=16; ++det_num){
		...
		fH1[aedet_str] = new TH1D(aedet_str.c_str(),aedet_str.c_str(),2000,0,2000);
		fH1[gedet_str] = new TH1D(gedet_str.c_str(),gedet_str.c_str(),2000,0,2000);
		fH1[ae2det_str] = new TH1D(ae2det_str.c_str(),ae2det_str.c_str(),2000,0,2000);
		fH1["aMult"] = new TH1D("aMult","aMult",20,0,20);
	...
	fH1["aE"] = new TH1D("aE", "Addback", 8000,0,2000);
	fH1["gE"] = new TH1D("gE", "Addback", 8000,0,2000);
	fH1["gEnoCT"] = new TH1D("gEnoCT", "Addback", 8000,0,2000);
	for(auto it : fH1) {
...
void CrossTalk::FillHistograms() {
   ...
   for(auto gr1 = 0; gr1 < fGrif->GetMultiplicity(); ++gr1){
		if(pileup_reject && (fGrif->GetGriffinHit(gr1)->GetKValue() != 700)) continue;
		...
		for(auto gr2 = gr1 + 1; gr2 < fGrif->GetMultiplicity(); ++gr2){
			if(pileup_reject && fGrif->GetGriffinHit(gr2)->GetKValue() != 700) continue;
			...
			//	std::cout << Form("det_%d_%d_%d",(int)(low_crys_hit->GetDetector()),(int)(low_crys_hit->GetCrystal()),(int)(high_crys_hit->GetCrystal())) << std::endl;
			//	std::cout << "Filling address: " << fH2[(Form("det_%d_%d_%d",(int)(low_crys_hit->GetDetector()),(int)(low_crys_hit->GetCrystal()),(int)(high_crys_hit->GetCrystal())))]<< std::endl;
				//fH2[Form("det_%d_%d_%d",low_crys_hit->GetDetector(),low_crys_hit->GetCrystal(),high_crys_hit->GetCrystal())]->Fill(10,10);
	...
	for(auto gr1 = 0; gr1 < fGrif->GetAddbackMultiplicity(); ++gr1){
		if(pileup_reject && (fGrif->GetAddbackHit(gr1)->GetKValue() != 700)) continue;
