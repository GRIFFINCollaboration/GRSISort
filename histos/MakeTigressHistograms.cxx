     ...
     */
      ...
      if(frag->GetChannelNumber() < 1199) {
         if(frag->GetSegment() == 0) {
            obj.FillHistogram("hpge", "core_energy", 70, 0, 70, frag->GetArrayNumber(), 8000, 0, 4000,
                              frag->GetEnergy());
            obj.FillHistogram("hpge", "core_charge", 70, 0, 70, frag->GetArrayNumber(), 10000, 0, 30000,
                              frag->GetCharge());
         }
      } else {
      }
      ...
      if(sharc_m > -1) {
         obj.FillHistogram("sharc", "mes_trigger_id", 100000, 0, 1000000, frag->GetTriggerId(), 40, 0, 40, sharc_m);
      }
   }
}
...
extern "C" void MakeAnalysisHistograms(TRuntimeObjects& obj)
{
   ...
   if(tigress) {
      ...
      for(UInt_t i = 0; i < tigress->GetMultiplicity(); i++) {
         obj.FillHistogram("gamma_energy", 4000, 1, 2001, tigress->GetTigressHit(i)->GetEnergy());
      }
   }
