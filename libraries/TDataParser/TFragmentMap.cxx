...
bool TFragmentMap::Add(TFragment* frag, std::vector<Int_t> charge, std::vector<Short_t> integrationLength) {
	...
	//std::cout<<"address "<<frag->GetAddress()<<": last fragment found, calculating charges for "<<nofFrags<<" fragments"<<std::endl;
	// last fragment:
		...
		case 2: // only one option: (2, 1)
			{
				...
				for(size_t i = 0; i < std::get<1>((*(range.first)).second).size(); ++i) { 
					if(k2[i] > 0) {
						...
						//std::cout<<"2, "<<i<<std::hex<<": 0x"<<std::get<1>((*(range.first)).second)[i]<<"/0x"<<k2[i]<<std::dec<<" = "<<(std::get<1>((*(range.first)).second)[i] + gRandom->Uniform())<<"/"<<k2[i]<<" = "<<c.back()<<std::endl;
					} else {
				...
				switch(dropped) {
					...
					default: // dropped none
						...
						//std::cout<<std::hex<<"2, -: 0x"<<charge[0]<<"/0x"<<integrationLength[0]<<std::dec<<" = "<<(charge[0] + gRandom->Uniform())<<"/"<<integrationLength[0]<<" = "<<c.back()<<std::endl;
		...
		case 3: // two options: (3, 1, 1), (2, 2, 1)
			{
				...
				for(size_t i = 0; i < std::get<1>((*(range.first)).second).size(); ++i) { 
					if(k2[i] > 0) {
						c.push_back((std::get<1>((*(range.first)).second)[i] + gRandom->Uniform())/k2[i]); 
						//std::cout<<"3, "<<i<<std::hex<<": 0x"<<std::get<1>((*(range.first)).second)[i]<<"/0x"<<k2[i]<<std::dec<<" = "<<(std::get<1>((*(range.first)).second)[i] + gRandom->Uniform())<<"/"<<k2[i]<<" = "<<c.back()<<std::endl;
					} else {
						dropped.push_back(i);
				...
				for(size_t i = 0; i < std::get<1>((*std::next(range.first)).second).size(); ++i) {
					if(k2[i+situation] > 0) {
						...
						//std::cout<<"3, "<<i+situation<<std::hex<<": 0x"<<std::get<1>((*std::next(range.first)).second)[i]<<"/0x"<<k2[i+situation]<<std::dec<<" = "<<(std::get<1>((*std::next(range.first)).second)[i] + gRandom->Uniform())<<"/"<<k2[i+situation]<<" = "<<c.back()<<std::endl;
					} else {
				...
				switch(dropped.size()) {
					case 0: // dropped none
						c.push_back((charge[0] + gRandom->Uniform())/integrationLength[0]);
						//std::cout<<std::hex<<"3, -: 0x"<<charge[0]<<"/0x"<<integrationLength[0]<<std::dec<<" = "<<(charge[0] + gRandom->Uniform())<<"/"<<integrationLength[0]<<" = "<<c.back()<<std::endl;
						//all k's are needed squared so we square all elements of k
						for(auto it = k2.begin(); it != k2.end(); ++it) { (*it) = (*it)*(*it); }
