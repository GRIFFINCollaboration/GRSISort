...
int TDataParser::GriffinDataToFragment(uint32_t* data, int size, int bank, unsigned int midasSerialNumber, time_t midasTime) {
  ...
  //Aug 19 updated to be data type 1 OR 4. We should put in a bank constraint or something later.
  if(((EventFrag->DataType == 1) || (EventFrag->DataType == 4)) && EventFrag->ChannelAddress == 0xFFFF){
    delete EventFrag;
