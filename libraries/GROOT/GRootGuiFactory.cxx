                                               ...
                                               Option_t* opt)
{
   ...
   if(pluginHandler != nullptr && pluginHandler->LoadPlugin() != -1) {
      auto* imp = reinterpret_cast<TBrowserImp*>(pluginHandler->ExecPlugin(5, b, title, width, height, browserOptions.Data()));
      if(imp != nullptr) {
                                               ...
                                               UInt_t height, Option_t* opt)
{
   ...
   if(pluginHandler != nullptr && pluginHandler->LoadPlugin() != -1) {
      auto* imp = reinterpret_cast<TBrowserImp*>(pluginHandler->ExecPlugin(7, b, title, x, y, width, height, browserOptions.Data()));
      if(imp != nullptr) {
         return imp;
      }
   }
