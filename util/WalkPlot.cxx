...
TH2F* timehistcfdnopu =
   new TH2F("time_engcfdnopu", "time_eng including CFD correction", 4000, -2000, 2000, 4000, 0, 4000);
...
void ProcessEvent(std::vector<TFragment>* event)
{
   ...
   for(size_t x = 0; x < event->size(); x++) {
      ...
      for(size_t y = 0; y < event->size(); y++) {
         ...
         Double_t timediff_walk = ((event->at(y).GetTime()) - (event->at(x).GetTime())) / 10.;
         ...
         if((event->at(x).GetDetectorType() == 0) && (event->at(y).GetDetectorType() == 0)) {
            ...
            if(event->at(x).GetEnergy() != 0.0) {
               timehist->Fill(timediff, event->at(y).GetEnergy());
               timehist_walk->Fill(timediff_walk, event->at(y).GetEnergy());
               timehist_chan->Fill(timediff, event->at(y).GetEnergy(),
                                   event->at(x).GetCrystal() + (event->at(y).GetDetector() - 1) * 4);
               timehist_chan2->Fill(timediff, event->at(y).GetEnergy(),
                                    event->at(y).GetCrystal() + (event->at(x).GetDetector() - 1) * 4);
