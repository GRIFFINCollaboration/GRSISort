...
TFragment::TFragment(const TFragment& rhs) : TVirtualFragment() {
	//copy constructor
...
TFragment& TFragment::operator=(const TFragment& rhs) {
...
Int_t TFragment::Get4GCfd(Int_t i) const { // return a 4G cfd in terms of 1/256 ns since the trigger
...
Float_t TFragment::GetCharge(Int_t i) const {
   TChannel* chan = TChannel::GetChannel(fChannelAddress);
   if(!chan)
      return 0.00;
...
double TFragment::GetEnergy(Int_t i) const {
   TChannel* chan = TChannel::GetChannel(fChannelAddress);
   if(!chan)
      return 0.00;
   if(chan->UseCalFileIntegration()) {
