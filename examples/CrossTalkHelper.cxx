...
void CrossTalkHelper::CreateHistograms(unsigned int slot)
{
   for(int det_num = 1; det_num <= 16; ++det_num) {
      ...
      H1()[slot][aedet_str]   = new TH1D(Form("aEdet%d", det_num), Form("Addback detector %d", det_num), 1500, 0, 1500);
      H1()[slot][gedet_str]   = new TH1D(Form("geEdet%d", det_num), Form("Singles detector %d", det_num), 1500, 0, 1500);
      H1()[slot][ae2det_str]  = new TH1D(Form("aE2det%d", det_num), Form("Addback with 2 hits, detector %d", det_num), 1500, 0, 1500);
      for(int crys_1 = 0; crys_1 < 4; ++crys_1) {
         for(int crys_2 = crys_1 + 1; crys_2 < 4; ++crys_2) {
            ...
            H2()[slot][name_str] = new TH2I(hist_name, hist_name, 1500, 0, 1500, 1500, 0, 1500);
            std::cout << " at address: " << H2()[slot][hist_name] << std::endl;
         }
      }
   }

   H1()[slot]["aMult"]   = new TH1D("aMult", "addback multilpicity", 20, 0, 20);
   H2()[slot]["gE_chan"] = new TH2D("gE_chan", "gE_chan", 65, 0, 65, 1500, 0, 1500);
   H1()[slot]["aE"]      = new TH1D("aE", "Summed Addback", 1500, 0, 1500);
   H1()[slot]["gE"]      = new TH1D("gE", "Summed Singles", 1500, 0, 1500);
   H1()[slot]["gEnoCT"]  = new TH1D("gEnoCT", "Singles, no CT correction", 1500, 0, 1500);
}
...
void CrossTalkHelper::Exec(unsigned int slot, TGriffin& grif, TGriffinBgo& grifBgo)
{
   ...
   for(auto gr1 = 0; gr1 < grif.GetSuppressedMultiplicity(&grifBgo); ++gr1) {
      ...
      H1()[slot].at(Form("gEdet%d", grif1->GetDetector()))->Fill(grif1->GetEnergy());
      H2()[slot].at("gE_chan")->Fill(grif1->GetArrayNumber(), grif1->GetEnergy());
      H1()[slot].at("gE")->Fill(grif1->GetEnergy());
      H1()[slot].at("gEnoCT")->Fill(grif1->GetNoCTEnergy());
      for(auto gr2 = gr1 + 1; gr2 < grif.GetSuppressedMultiplicity(&grifBgo); ++gr2) {
         ...
         if((detMultiplicity[grif1->GetDetector()] == 2) && Addback(grif1, grif2)) {
            ...
            if(low_crys_hit->GetCrystal() != high_crys_hit->GetCrystal()) {
               H2()[slot].at(Form("det_%d_%d_%d", low_crys_hit->GetDetector(), low_crys_hit->GetCrystal(), high_crys_hit->GetCrystal()))->Fill(low_crys_hit->GetNoCTEnergy(), high_crys_hit->GetNoCTEnergy());
            }
         }
      }
   }
   ...
   for(auto gr1 = 0; gr1 < grif.GetSuppressedAddbackMultiplicity(&grifBgo); ++gr1) {
      ...
      if(pileup_reject && (grif1->GetKValue() != defaultKValue)) continue;   // This pileup number might have to change for other expmnts
      H1()[slot].at("aE")->Fill(grif1->GetEnergy());
      H1()[slot].at(Form("aEdet%d", grif1->GetDetector()))->Fill(grif1->GetEnergy());
      H1()[slot].at("aMult")->Fill(grif.GetNSuppressedAddbackFrags(gr1));
      if(grif.GetNSuppressedAddbackFrags(gr1) == 2) H1()[slot].at(Form("aE2det%d", grif1->GetDetector()))->Fill(grif1->GetEnergy());
   }
}
