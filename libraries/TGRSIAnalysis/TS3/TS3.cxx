							...
							if(abs(fS3RingHits[i].GetCfd() - fS3SectorHits[j].GetCfd()) < fFrontBackTime &&
								...
								if(EneR[i] * fFrontBackEnergy < (EneS[j] + EneS[k]) &&
									...
									if(abs(SectorSep) == 1 || abs(SectorSep) == fSectorNumber) {
										...
										if(KeepShared()) {
											TS3Hit dethit =
												fS3RingHits[i]; // Ring defines all data sector just gives position
										...
										// Now we have accepted a good event, build it
										TS3Hit dethit =
											fS3SectorHits[j]; // Sector now defines all data ring just gives position
										dethit.SetRingNumber(fS3RingHits[i].GetRing());
										fS3Hits.push_back(dethit);

										// Now we have accepted a good event, build it
										TS3Hit dethitB =
											fS3SectorHits[k]; // Sector now defines all data ring just gives position
					...
					for(size_t j = 0; j < fS3RingHits.size(); ++j) {
						...
						for(size_t k = j + 1; k < fS3RingHits.size(); ++k) {
							...
							if(abs(fS3SectorHits[i].GetCfd() - fS3RingHits[j].GetCfd()) < fFrontBackTime &&
									abs(fS3SectorHits[i].GetCfd() - fS3RingHits[k].GetCfd()) <
									fFrontBackTime) { // first check time
								if(EneS[i] * fFrontBackEnergy < (EneR[j] + EneR[k]) &&
										(EneR[j] + EneR[k]) * fFrontBackEnergy <
										EneS[i]) { // if time is good check energy
									...
									if(abs(fS3RingHits[j].GetRing() - fS3RingHits[k].GetRing()) == 1) {
										...
										if(KeepShared()) {
											TS3Hit dethit =
												fS3SectorHits[i]; // Sector defines all data ring just gives position
										...
										// Now we have accepted a good event, build it
										TS3Hit dethit =
											fS3RingHits[j]; // Ring defines all data sector just gives position
										dethit.SetSectorNumber(fS3SectorHits[i].GetSector());
										fS3Hits.push_back(dethit);

										// Now we have accepted a good event, build it
										TS3Hit dethitB =
											fS3RingHits[k]; // Ring defines all data sector just gives position
...
TVector3 TS3::GetPosition(int ring, int sector, double offsetphi, double offsetZ, bool sectorsdownstream,
		bool smear)
{
...
TGRSIDetectorHit* TS3::GetHit(const int& idx) { return GetS3Hit(idx); }
...
void TS3::Print(Option_t*) const { printf("%s\tnot yet written.\n", __PRETTY_FUNCTION__); }
