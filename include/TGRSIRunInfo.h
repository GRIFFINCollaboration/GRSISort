...
class TGRSIRunInfo : public TObject {
   ...
   public:
      ...
      inline void SetAddBackWindow(const double   t_abw) { fAddBackWindow = t_abw; } 
		...
		inline void SetBufferSize(const int t_bs)          { fBufferSize = t_bs; }
      ...
      static inline double   AddBackWindow()  { return Get()->fAddBackWindow; }
		...
		static inline int      BufferSize()     { return Get()->fBufferSize; }
   ...
   private:
      ...
      int fBufferSize;                // GRIFFIN: the minimum size of the sorting buffer (default = 1 000 000)
