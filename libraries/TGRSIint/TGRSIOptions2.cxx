...
TGRSIOptions2* TGRSIOptions2::Get(int argc, char** argv){
  static TGRSIOptions2* item = NULL;
  if(!item){
    item = new TGRSIOptions2(argc, argv);
  }
  return item;
}

TGRSIOptions2::TGRSIOptions2(int argc, char** argv)
  : fShouldExit(false) {
  Load(argc, argv);
}

void TGRSIOptions2::Clear(Option_t* opt) {
...
void TGRSIOptions2::Load(int argc, char** argv) {
  ...
  parser.option("suppress-error", &fSuppressErrors);
  parser.option("suppress-errors", &fSuppressErrors);
  parser.option("suppress_error", &fSuppressErrors);
  parser.option("suppress_errors", &fSuppressErrors);
...
kFileType TGRSIOptions2::DetermineFileType(const std::string& filename) const{
...
bool TGRSIOptions2::FileAutoDetect(const std::string& filename) {
...
std::string TGRSIOptions2::GenerateOutputFilename(const std::string& filename){
  return "temp.root";
}

std::string TGRSIOptions2::GenerateOutputFilename(const std::vector<std::string>& filename){
