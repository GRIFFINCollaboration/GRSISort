...
void ExampleEventHelper::CreateHistograms(unsigned int slot)
{
   ...
   H2()[slot]["zdsMultGriffinMult"] = new TH2I("zdsMultGriffinMult", "ZDS multiplicity vs. GRIFFIN multiplicity (unsuppressed)", 65, -0.5, 64.5, 10, -0.5, 9.5);
   ...
   H1()[slot]["griffinESuppAddbackBeta"]         = new TH1F("griffinESuppAddbackBeta", Form("Suppressed griffin addback energy w/ #beta-tag;energy [keV];counts/%.1f keV", (highEnergy - lowEnergy) / energyBins), energyBins, lowEnergy, highEnergy);
   H2()[slot]["griffinESuppAddbackMatrixBeta"]   = new TH2F("griffinESuppAddbackMatrixBeta", "Suppressed griffin addback energy matrix w/ #beta-tag;energy [keV];energy [keV]", energyBins / 5, lowEnergy, highEnergy, energyBins / 5, lowEnergy, highEnergy);
   H2()[slot]["griffinESuppAddbackMatrixBetaBg"] = new TH2F("griffinESuppAddbackMatrixBetaBg", "Suppressed griffin addback energy matrix w/ #beta-tag (time random BG);energy [keV];energy [keV]", energyBins / 5, lowEnergy, highEnergy, energyBins / 5, lowEnergy, highEnergy);

   // timing spectra
   H2()[slot]["griffinZdsTS"]       = new TH2F("griffinZdsTS", "GRIFFIN crystal vs. GRIFFIN-ZDS timestamp difference (suppressed addback);#DeltaTS_{GRIFFIN-ZDS}", 200, -1000., 1000., 64, 0.5, 64.5);
   H2()[slot]["griffinZdsTime"]     = new TH2F("griffinZdsTime", "GRIFFIN crystal vs. GRIFFIN-ZDS timing (suppressed addback);#Deltat_{GRIFFIN-ZDS}", 2000, -1000., 1000., 64, 0.5, 64.5);
   H2()[slot]["griffinGriffinTS"]   = new TH2F("griffinGriffinTS", "GRIFFIN crystal vs. GRIFFIN-GRIFFIN timestamp difference (suppressed addback);#DeltaTS_{GRIFFIN-GRIFFIN}", 2000, -1000., 1000., 64, 0.5, 64.5);
   H2()[slot]["griffinGriffinTime"] = new TH2F("griffinGriffinTime", "GRIFFIN crystal vs. GRIFFIN-GRIFFIN timing (suppressed addback);#Deltat_{GRIFFIN-GRIFFIN}", 2000, -1000., 1000., 64, 0.5, 64.5);

   // cycle spectra
   if(fCycleLength > 0) {
      H2()[slot]["griffinCycle"] = new TH2F("griffinCycle", "GRIFFIN suppressed addback energy w/ #beta-tag vs. time in cycle;time in cycle [s];energy [keV]", 100 * fCycleLength / 1e6, 0., fCycleLength / 1e6, energyBins / 5, lowEnergy, highEnergy);
      H1()[slot]["zdsCycle"]     = new TH1F("zdsCycle", "ZDS hits in cycle;time in cycle [s]", 100 * fCycleLength / 1e6, 0., fCycleLength / 1e6);
   }
}
...
void ExampleEventHelper::Exec(unsigned int slot, TGriffin& grif, TGriffinBgo& grifBgo, TZeroDegree& zds) // NOLINT
{
   ...
   // multiplicities
   H2()[slot].at("zdsMultGriffinMult")->Fill(grif.GetMultiplicity(), zds.GetMultiplicity());
   ...
   for(int g = 0; g < grif.GetSuppressedAddbackMultiplicity(&grifBgo); ++g) {
      ...
      for(int z = 0; z < zds.GetMultiplicity(); ++z) {
         ...
         H2()[slot].at("griffinZdsTS")->Fill(grif1->GetTimeStampNs() - zds1->GetTimeStampNs(), grif1->GetArrayNumber());
         H2()[slot].at("griffinZdsTime")->Fill(grif1->GetTime() - zds1->GetTime(), grif1->GetArrayNumber());
         if(PromptCoincidence(grif1, zds1)) promptBeta = true;
      }
      if(promptBeta) {
         H1()[slot].at("griffinESuppAddbackBeta")->Fill(grif1->GetEnergy());
         if(fCycleLength > 0.) {
            H2()[slot].at("griffinCycle")->Fill(std::fmod(grif1->GetTime(), fCycleLength), grif1->GetEnergy());
         }
         for(int g2 = g + 1; g2 < grif.GetSuppressedAddbackMultiplicity(&grifBgo); ++g2) {
            auto grif2 = grif.GetSuppressedAddbackHit(g2);
            H2()[slot].at("griffinGriffinTS")->Fill(grif1->GetTimeStampNs() - grif2->GetTimeStampNs(), grif1->GetArrayNumber());
            H2()[slot].at("griffinGriffinTime")->Fill(grif1->GetTime() - grif2->GetTime(), grif1->GetArrayNumber());
            if(PromptCoincidence(grif1, grif2)) {
               // fill twice to get a symmetric matrix
               H2()[slot].at("griffinESuppAddbackMatrixBeta")->Fill(grif1->GetEnergy(), grif2->GetEnergy());
               H2()[slot].at("griffinESuppAddbackMatrixBeta")->Fill(grif2->GetEnergy(), grif1->GetEnergy());
            } else if(TimeRandom(grif1, grif2)) {
               // fill twice to get a symmetric matrix
               H2()[slot].at("griffinESuppAddbackMatrixBetaBg")->Fill(grif1->GetEnergy(), grif2->GetEnergy());
               H2()[slot].at("griffinESuppAddbackMatrixBetaBg")->Fill(grif2->GetEnergy(), grif1->GetEnergy());
            }
         }
      }
   }
   if(fCycleLength > 0.) {
      for(int z = 0; z < zds.GetMultiplicity(); ++z) {
         auto zds1 = zds.GetZeroDegreeHit(z);
         H1()[slot].at("zdsCycle")->Fill(std::fmod(zds1->GetTime(), fCycleLength));
      }
   }
}
...
{
   auto coincident = static_cast<TH2*>(list->at("").FindObject(H2()[0].at("griffinESuppAddbackMatrixBeta")));
   if(coincident == nullptr) {
      std::cout << "Failed to find griffinESuppAddbackMatrixBeta histogram in list:" << std::endl;
      list->at("").Print();
      return;
   }
   auto timeRandom = static_cast<TH2*>(list->at("").FindObject(H2()[0].at("griffinESuppAddbackMatrixBetaBg")));
