// g++ offsetadd.cxx `root-config --cflags --libs` -I${GRSISYS}/include -L${GRSISYS}/libraries -lMidasFormat -lXMLParser
// -ooffsetadd
...
int main(int argc, char** argv)
{
   ...
   while(infile->Read(event) > 0) {
      ...
      case 0x8001:
         ...
         outfile->Write(event);
         break;
      default: ProcessEvent(event, outfile); break;
      };
   }
   infile->Close();
   outfile->Close();
...
void ProcessEvent(const std::shared_ptr<TMidasEvent>& event, TMidasFile* outfile)
{
   ...
   if(((chanadd & 0x0000ff00) == 0x00000000) || ((chanadd & 0x0000ff00) == 0x00001000) ||
      ((chanadd & 0x0000ff00) == 0x00001100) || ((chanadd & 0x0000ff00) == 0x00001200)) {
      // do nothing.
   } else {
      outfile->Write(event, "q");
      return;
