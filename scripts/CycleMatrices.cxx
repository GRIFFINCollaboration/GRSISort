...
// g++ CycleMatrices.cxx -std=c++0x -I$GRSISYS/include -L$GRSISYS/libraries -lAnalysisTreeBuilder -lGriffin -lSceptar
...
TList* CycleMatrices(TTree* tree, TPPG* ppg, TGRSIRunInfo* runInfo, long maxEntries = 0, TStopwatch* w = nullptr)
{
   ...
   THnSparseF* gSinglesCyc_chan_corr = dynamic_cast<THnSparseF*>(gSinglesCyc_chan->Clone("gSinglesCyc_chan_corr"));
   THnSparseF* pileup_corr           = dynamic_cast<THnSparseF*>(pileup->Clone("pileup_corr"));
   ...
   float            average = (static_cast<float>(sum)) / nofCycles;
   auto*            coord   = new Int_t[4];
   int              removed = 0;
   ...
   for(int i = cyc->GetNbinsX(); i >= 2; --i) {
      if(last && cyc->GetBinContent(i) > 0) {
         std::cout<<"Removing last cycle "<<i<<": average = "<<average<<", cycle = "<<cyc->GetBinContent(i)
                  <<std::endl;
   ...
   for(int i = 1; i <= cyc->GetNbinsX(); ++i) {
      ...
      if(first && cyc->GetBinContent(i) > 0) {
         std::cout<<"Removing first cycle "<<i<<": average = "<<average<<", cycle = "<<cyc->GetBinContent(i)
                  <<std::endl;
         removed += cyc->GetBinContent(i);
         --nofCycles;
         cyc->SetBinContent(i, 0);
      ...
      if(cyc->GetBinContent(i) > 1.5 * average) {
         std::cout<<badCycles.size()<<". bad cycle "<<i<<": average = "<<average
                  <<", cycle = "<<cyc->GetBinContent(i)<<std::endl;
         removed += cyc->GetBinContent(i);
         --nofCycles;
         cyc->SetBinContent(i, 0);
         ...
         if(i == lastGoodCycle) {
            ...
            for(int j = 1; j < i; ++j) {
               ...
               if(i - j == firstGoodCycle || badCycles.empty()) {
                  lastGoodCycle = i - j;
                  break;
               }
   ...
   average = (static_cast<float>(sum - removed)) / nofCycles;
   ...
   auto* goodCycle = new bool[cyc->GetNbinsX()];
