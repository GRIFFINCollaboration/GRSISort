...
// we have to repeat the code from the default constructor here, because calling the TH1 constructor and the GHSym default constructor gives an error
...
Bool_t GHSym::Add(TF1* f1, Double_t c1, Option_t* option) {
...
Bool_t GHSym::Add(const TH1* h1, Double_t c1) {
...
Bool_t GHSym::Add(const TH1* h1, const TH1* h2, Double_t c1, Double_t c2) {
...
Double_t GHSym::DoIntegral(Int_t binx1, Int_t binx2, Int_t biny1, Int_t biny2, Double_t& error, Option_t* option, Bool_t doError) const {
	...
	for(Int_t binx = binx1; binx <= binx2; ++binx) {
		...
		for(Int_t biny = biny1; biny <= biny2; ++biny) {
			...
			if(width) integral += GetBinContent(bin)*dx*dy;
			else      integral += GetBinContent(bin);
			if(doError) {
				if(width)  igerr2 += GetBinError(bin)*GetBinError(bin)*dx*dx*dy*dy;
...
void GHSym::FillRandom(const char* fname, Int_t ntimes) {
	...
	if(fobj == nullptr) { Error("FillRandom", "Unknown function: %s",fname); return; }
	TF2* f1 = dynamic_cast<TF2*>(fobj);
	if(f1 == nullptr) { Error("FillRandom", "Function: %s is not a TF2",fname); return; }
...
void GHSym::FillRandom(TH1* h, Int_t ntimes) {
	...
	if(h == nullptr) { Error("FillRandom", "Null histogram"); return; }
...
void GHSym::FitSlices(TF1* f1, Int_t firstbin, Int_t lastbin, Int_t cut, Option_t* option, TObjArray* arr) {
	...
	if(f1 == nullptr) {
		...
		if(f1 == nullptr) f1 = new TF1("gaus", "gaus", fXaxis.GetXmin(), fXaxis.GetXmax());
...
void GHSym::GetStats(Double_t* stats) const {
	...
	if(fBuffer) const_cast<GHSym*>(this)->BufferEmpty();
...
Double_t GHSym::IntegralAndError(Int_t firstxbin, Int_t lastxbin, Int_t firstybin, Int_t lastybin, Double_t & error, Option_t* option) const {
	//Return integral of bin contents in range [firstxbin,lastxbin],[firstybin,lastybin]
...
Double_t GHSym::Interpolate(Double_t x, Double_t y) {
	...
	if(dx <= fXaxis.GetBinWidth(bin_x)/2 && dy <= fYaxis.GetBinWidth(bin_y)/2) quadrant = 1; // upper right
	if(dx >  fXaxis.GetBinWidth(bin_x)/2 && dy <= fYaxis.GetBinWidth(bin_y)/2) quadrant = 2; // upper left
	if(dx >  fXaxis.GetBinWidth(bin_x)/2 && dy >  fYaxis.GetBinWidth(bin_y)/2) quadrant = 3; // lower left
	if(dx <= fXaxis.GetBinWidth(bin_x)/2 && dy >  fYaxis.GetBinWidth(bin_y)/2) quadrant = 4; // lower right
	switch(quadrant) {
		case 1:
...
Double_t GHSym::KolmogorovTest(const TH1* h2, Option_t* option) const {
	...
	else afunc1 = kTRUE;    // use later for calculating z

	if(w2 > 0) esum2 = sum2 * sum2 / w2;
	else afunc2 = kTRUE;    // use later for calculating z
...
TProfile* GHSym::Profile(const char* name, Int_t firstbin, Int_t lastbin, Option_t* option) const {
	...
	if(h1obj && h1obj->InheritsFrom(TH1::Class())) {
		...
		if(xbins->fN == 0) {
			if(originalRange) h1->SetBins(fXaxis.GetNbins(), fXaxis.GetXmin(), fXaxis.GetXmax());
			else  				h1->SetBins(lastOutBin-firstOutBin+1, fXaxis.GetBinLowEdge(firstOutBin), fXaxis.GetBinUpEdge(lastOutBin));
		} else {
			// case variable bins
			if(originalRange) h1->SetBins(fXaxis.GetNbins(), xbins->fArray);
			else  				h1->SetBins(lastOutBin-firstOutBin+1, &xbins->fArray[firstOutBin-1]);
		}
	}
	...
	if(h1 == nullptr) {
		...
		if(bins->fN == 0) {
			if(originalRange)	h1 = new TProfile(pname, GetTitle(), fXaxis.GetNbins(), fXaxis.GetXmin(), fXaxis.GetXmax(), opt);
			else	            h1 = new TProfile(pname, GetTitle(), lastOutBin-firstOutBin+1,
					fXaxis.GetBinLowEdge(firstOutBin),
					fXaxis.GetBinUpEdge(lastOutBin), opt);
		} else {
			// case variable bins
			if(originalRange)	h1 = new TProfile(pname, GetTitle(), fXaxis.GetNbins(), bins->fArray, opt);
			else  				h1 = new TProfile(pname, GetTitle(), lastOutBin-firstOutBin+1, &bins->fArray[firstOutBin-1], opt);
		}
	}
	...
	for(Int_t outbin = 0; outbin <= fXaxis.GetNbins() + 1;  ++outbin) {
		if(fXaxis.TestBit(TAxis::kAxisRange) && (outbin < firstOutBin || outbin > lastOutBin)) continue;
...
GHSym* GHSym::Rebin2D(Int_t ngroup, const char* newname) {
	...
	Double_t* oldBins = new Double_t[(nbins+2)*(nbins+3)/2];
			...
			for(ybin = 1; ybin <= xbin; ++ybin) {
				...
				for(i = 0; i < ngroup; ++i) {
					...
					for(j = 0; j < ngroup; ++j) {
						...
						if(oldybin+j <= oldxbin+i)	bin = oldxbin+i + (oldybin+j)*(2*fXaxis.GetNbins()-(oldybin+j)+3)/2;
						else                       bin = oldybin+j + (oldxbin+i)*(2*fXaxis.GetNbins()-(oldxbin+i)+3)/2;
...
TH1* GHSym::ShowBackground(Int_t niter, Option_t* option) {
	...
	return reinterpret_cast<TH1*>(gROOT->ProcessLineFast(Form("TSpectrum2::StaticBackground((TH1*)0x%lx,%d,\"%s\")",
...
TH2D* GHSymD::GetMatrix(bool force) {
	...
	fMatrix = new TH2D(Form("%s_mat", GetName()), Form("%s;%s;%s", GetTitle(), fXaxis.GetTitle(), fYaxis.GetTitle()),
			             fXaxis.GetNbins(), fXaxis.GetXmin(), fXaxis.GetXmax(), fYaxis.GetNbins(), fYaxis.GetXmin(), fYaxis.GetXmax());
	//copy cell contents (including all overflow and underflow cells)
	for(int i = 0; i < fXaxis.GetNbins()+2; ++i) {
		for(int j = 0; j < fXaxis.GetNbins()+2; ++j) {
			fMatrix->SetBinContent(i,j,GetBinContent(i,j));
