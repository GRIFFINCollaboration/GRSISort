...
TList *AnalyzeFragmentTree(TTree *tree, long entries = 0, TStopwatch* w = NULL) {
  ...
  if(w == NULL) {
   ...
   TH1F* gamma = new TH1F("gamma","#gamma;energy [keV];counts/keV",16000,0.,16000.); list->Add(gamma);
   TH1F* betaGamma = new TH1F("betaGamma","#beta#gamma;energy [keV];counts/keV",16000,0.,16000.); list->Add(betaGamma);
   ...
   TH1F *gg_diff = new TH1F("gg_diff","gg_diff",maxTime-minTime, minTime, maxTime); list->Add(gg_diff);
   TH1F *gb_diff = new TH1F("gb_diff","gb_diff",maxTime-minTime, minTime, maxTime); list->Add(gb_diff);
   TH1F *bg_diff = new TH1F("bg_diff","bg_diff",maxTime-minTime, minTime, maxTime); list->Add(bg_diff);
   TH1F *bb_diff = new TH1F("bb_diff","bb_diff",maxTime-minTime, minTime, maxTime); list->Add(bb_diff);
