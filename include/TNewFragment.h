...
#ifndef TNEWFRAGMENT_H
#define TNEWFRAGMENT_H
...
class TNewFragment : public TFragment	{
public:
   TNewFragment(); 
	TNewFragment(const TNewFragment&); ///< copy constructor
   virtual ~TNewFragment(); 
...
public:  
	...
	void SetNumberOfFilters(UShort_t value) { std::cerr<<"Error, "<<__PRETTY_FUNCTION__<<" called, TNewFragment shouldn't have a number of filters."<<std::endl; }
   ...
   void Print(Option_t *opt = "") const;
	...
	TNewFragment* Clone(const char* name = "");
   
   bool operator<(const TNewFragment& rhs) const { return (GetTimeStamp() < rhs.GetTimeStamp()); }
   bool operator>(const TNewFragment& rhs) const { return (GetTimeStamp() > rhs.GetTimeStamp()); }

	TNewFragment& operator=(const TNewFragment&);

/// \cond CLASSIMP
   ClassDef(TNewFragment,1);  // Event Fragments
/// \endcond
};
/*! @} */
#endif // TNEWFRAGMENT_H
