...
// g++ exAnalysis.C -std=c++0x -I$GRSISYS/include -L$GRSISYS/libraries -lAnalysisTreeBuilder -lGriffin -lSceptar
...
TList* exAnalysis(TTree* tree, TPPG*, TGRSIRunInfo*, long maxEntries = 0, TStopwatch* w = nullptr)
{
   ...
   auto*  ggbmatrix                 = new THnSparseF("ggbmatrix", "#gamma-#gamma-#beta matrix", 2, bins, min, max);
   list->Add(ggbmatrix);
   auto* ggbmatrix_bg = new THnSparseF("ggbmatrix_bg", "#gamma-#gamma-#beta background matrix", 2, bins, min, max);
   list->Add(ggbmatrix_bg);
   for(int i = 0; i < static_cast<int>(angleCombinationsab.size()); ++i) {
      //  angCorr_coinc_Binnedab[i] = new TH2F(Form("angCorr_coinc_Binnedab_%d",i),Form("angular correlation at %.1f
      //  ^{o} addback on beam window;energy [keV];energy [keV]",angleCombinationsab[i].first), 1500, xBins, 1500,
      //  yBins); list->Add(angCorr_coinc_Binnedab[i]);
      // angCorr_coinc_Binnedab[i] = new TH2F(Form("angCorr_coinc_Binnedab_%d",i),Form("angular correlation at %.1f ^{o}
      // addback on beam window;energy [keV];energy [keV]",angleCombinationsab[i].first), nofBins, low, high,
      // nofBins,low,high); list->Add(angCorr_coinc_Binnedab[i]);
      angCorr_coinc_Binnedab[i] =
         new THnSparseF(Form("angCorr_coinc_Binnedab_%d", i),
                        Form("angular correlation at %.1f ^{o} addback on beam window;energy [keV];energy [keV]",
                             angleCombinationsab[i].first),
                        2, bins, min, max);
      list->Add(angCorr_coinc_Binnedab[i]);
      angCorr_coinc_Binnedab_bg[i] = new THnSparseF(
         Form("angCorr_coinc_Binnedab_bg_%d", i),
         Form("angular correlation at %.1f ^{o} addback on beam window, time randoms;energy [keV];energy [keV]",
              angleCombinationsab[i].first),
         2, bins, min, max);
      list->Add(angCorr_coinc_Binnedab_bg[i]);
   ...
   for(int i = 0; i < static_cast<int>(angleCombinations.size()); ++i) {
      //  angCorr_coinc_Binned[i] = new TH2F(Form("angCorr_coinc_Binned_%d",i),Form("angular correlation at %.1f ^{o} on
      //  beam window;energy [keV];energy [keV]",angleCombinations[i].first), 1500, xBins, 1500, yBins);
      //  list->Add(angCorr_coinc_Binned[i]);
      // angCorr_coinc_Binned[i] = new TH2F(Form("angCorr_coinc_Binned_%d",i),Form("angular correlation at %.1f ^{o} on
      // beam window;energy [keV];energy [keV]",angleCombinations[i].first), nofBins, low, high,nofBins,low,high);
      // list->Add(angCorr_coinc_Binned[i]);
      angCorr_coinc_Binned[i] = new THnSparseF(
         Form("angCorr_coinc_Binned_%d", i),
         Form("angular correlation at %.1f ^{o} on beam window;energy [keV];energy [keV]", angleCombinations[i].first),
         2, bins, min, max);
      list->Add(angCorr_coinc_Binned[i]);
      angCorr_coinc_Binned_bg[i] =
         new THnSparseF(Form("angCorr_coinc_Binned_bg_%d", i),
                        Form("angular correlation at %.1f ^{o} on beam window, time randoms;energy [keV];energy [keV]",
                             angleCombinations[i].first),
                        2, bins, min, max);
      list->Add(angCorr_coinc_Binned_bg[i]);
      angCorr_coinc_Binned_uncorr[i] =
         new THnSparseF(Form("angCorr_coinc_Binned_uncorr_%d", i),
                        Form("angular correlation at %.1f ^{o} on beam window (uncorrelated);energy [keV];energy [keV]",
                             angleCombinations[i].first),
                        2, bins, min, max);
      list->Add(angCorr_coinc_Binned_uncorr[i]);
      angleComboMap.insert(std::make_pair(angleCombinations[i].first, i));
   }

   // Double_t tlow[2] = {0,0};
   // Double_t thigh[2] = {200,200};
   auto* bggTimeDiff = new TH2D("bggTimeDiff", "Time Difference;t_{#gamma2}-t_{#beta};t_{#gamma1}-t_{#beta};", 400,
                                -200., 200., 400, -200., 200.);
   list->Add(bggTimeDiff);
   auto* bggTimeDiffLargeE =
      new TH2D("bggTimeDiffLargeE", "Time Difference, E_{#gamma} > 1 MeV;t_{#gamma2}-t_{#beta};t_{#gamma1}-t_{#beta};",
               400, -200., 200., 400, -200., 200.);
   list->Add(bggTimeDiffLargeE);
         ...
         for(int b = 0; b < scep->GetMultiplicity() && !foundBeta; ++b) {
            ...
            for(one = 0; one < (int)grif->GetMultiplicity(); ++one) {
               ...
               for(two = 0; two < (int)grif->GetMultiplicity(); ++two) {
                  if(grif->GetGriffinHit(two)->NPileUps() > 1) {
                     continue;
                  }
                  if(two == one) { // If we are looking at the same gamma we don't want to call it a coincidence
                     continue;
                  }
                  bggTimeDiff->Fill(grif->GetGriffinHit(one)->GetTime() - scep->GetSceptarHit(b)->GetTime(),
                                    grif->GetGriffinHit(two)->GetTime() - scep->GetSceptarHit(b)->GetTime());
                  if(grif->GetGriffinHit(one)->GetEnergy() > 1000. && grif->GetGriffinHit(two)->GetEnergy() > 1000.) {
                     bggTimeDiffLargeE->Fill(grif->GetGriffinHit(one)->GetTime() - scep->GetSceptarHit(b)->GetTime(),
                                             grif->GetGriffinHit(two)->GetTime() - scep->GetSceptarHit(b)->GetTime());
                  }
                  foundBeta = true;
                  ...
                  if(((gbTlow <= grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
                     ...
                     ((gbTlow <= grif->GetHit(two)->GetTime() - scep->GetHit(b)->GetTime()) &&
                      (grif->GetHit(two)->GetTime() - scep->GetHit(b)->GetTime() <= gbThigh)) &&
                     ((ggTlow <= TMath::Abs(grif->GetHit(one)->GetTime() - grif->GetHit(two)->GetTime())) &&
                      (TMath::Abs(static_cast<Int_t>(grif->GetHit(one)->GetTime() - grif->GetHit(two)->GetTime() <=
                                                     ggThigh)) != 0))) {
                     ...
                     double angle =
                        grif->GetGriffinHit(one)->GetPosition().Angle(grif->GetGriffinHit(two)->GetPosition()) * 180. /
                        TMath::Pi();
                     if(angle < 0.0001) {
                        continue;
                     }
                     auto angIndex = angleComboMap.lower_bound(angle - 0.0005);
                     // angCorr_coinc_Binned[angIndex->second]->Fill(grif->GetGriffinHit(one)->GetEnergy(),
                     // grif->GetGriffinHit(two)->GetEnergy(), 1.);
                     Double_t fillval[2] = {grif->GetGriffinHit(one)->GetEnergy(),
                                            grif->GetGriffinHit(two)->GetEnergy()};
                     angCorr_coinc_Binned[angIndex->second]->Fill(fillval);
                  } else if((((gbBGlow <= grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
                              (grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime() <= gbBGhigh)) &&
                             ((gbTlow <= grif->GetHit(two)->GetTime() - scep->GetHit(b)->GetTime()) &&
                              (grif->GetHit(two)->GetTime() - scep->GetHit(b)->GetTime() <= gbThigh))) ||
                            (((gbBGlow <= grif->GetHit(two)->GetTime() - scep->GetHit(b)->GetTime()) &&
                              (grif->GetHit(two)->GetTime() - scep->GetHit(b)->GetTime() <= gbBGhigh)) &&
                             ((gbTlow <= grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
                              (grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime() <= gbThigh)))) {
                     // Plots a gamma energy spectrum in coincidence with a beta
                     // Now we want to loop over gamma rays if they are in coincidence.
                     // If they are close enough in time, fill the gamma-gamma-beta matrix. This will be symmetric
               ...
               if(oldgrif != nullptr) {
                  bool oldfound = false;
                  for(int oldb = 0; (oldb < static_cast<int>(oldscep->GetMultiplicity())) && !oldfound; ++oldb) {
                     // Be careful about time ordering!!!! betas and gammas are not symmetric out of the DAQ
                     for(int oldtwo = 0; oldtwo < (int)oldgrif->GetMultiplicity(); ++oldtwo) {
                        if(oldgrif->GetGriffinHit(oldtwo)->NPileUps() > 1) {
                           continue;
                        }
                        if(oldtwo == one) {
                           continue;
                        ...
                        if(((gbTlow <= grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
                            (grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime() <= gbThigh)) &&
                           ((gbTlow <= oldgrif->GetHit(oldtwo)->GetTime() - oldscep->GetHit(oldb)->GetTime()) &&
                            (oldgrif->GetHit(oldtwo)->GetTime() - oldscep->GetHit(oldb)->GetTime() <= gbThigh))) {
                           // Plots a gamma energy spectrum in coincidence with a beta
                           // Now we want to loop over gamma rays if they are in coincidence.
                           // If they are close enough in time, fill the gamma-gamma-beta matrix. This will be symmetric
                           // because we are doing a double loop over gammas
                           double angle = grif->GetGriffinHit(one)->GetPosition().Angle(
                                             oldgrif->GetGriffinHit(oldtwo)->GetPosition()) *
                                          180. / TMath::Pi();
                           if(angle < 0.0001) {
                              continue;
                           }
                           ...
                           // grif->GetGriffinHit(two)->GetEnergy(), 1.);
                           Double_t fillval[2] = {grif->GetGriffinHit(one)->GetEnergy(),
                                                  oldgrif->GetGriffinHit(oldtwo)->GetEnergy()};
                           angCorr_coinc_Binned_uncorr[angIndex->second]->Fill(fillval);
                           Double_t fillvalswitched[2] = {oldgrif->GetGriffinHit(oldtwo)->GetEnergy(),
            ...
            for(int b = 0; b < scep->GetMultiplicity(); ++b) {
               ...
               for(two = 0; two < (int)grif->GetAddbackMultiplicity(); ++two) {
                  ...
                  if(((gbTlow <= grif->GetAddbackHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
                     ...
                     ((gbTlow <= grif->GetAddbackHit(two)->GetTime() - scep->GetHit(b)->GetTime()) &&
                      (grif->GetAddbackHit(two)->GetTime() - scep->GetHit(b)->GetTime() <= gbThigh))) {
                     double angab =
                        grif->GetAddbackHit(one)->GetPosition().Angle(grif->GetAddbackHit(two)->GetPosition()) * 180. /
                        TMath::Pi();
                     auto angIndex = angleComboMapab.lower_bound(angab - 0.005);
                     //                             angCorr_coinc_Binnedab[angIndex->second]->Fill(grif->GetAddbackHit(one)->GetEnergy(),
                     //                             grif->GetAddbackHit(two)->GetEnergy(), 1.);
                     Double_t fillval[2] = {grif->GetAddbackHit(one)->GetEnergy(),
                                            grif->GetAddbackHit(two)->GetEnergy()};
                     angCorr_coinc_Binnedab[angIndex->second]->Fill(fillval);
                     //                             angCorr_coinc_Binnedab[angIndexab]->Fill(grif->GetAddbackHit(one)->GetEnergy(),
                     //                             grif->GetAddbackHit(two)->GetEnergy(), 1.);
                  } else if(((gbBGlow <= grif->GetAddbackHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
