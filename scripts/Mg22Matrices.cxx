...
// g++ Mg22Matrices.cxx -std=c++0x -I$GRSISYS/include -L$GRSISYS/libraries -lAnalysisTreeBuilder -lGriffin -lSceptar
...
//  2. Calls the Mg22Matrices function
//  3. Mg22Matrices creates 1D and 2D histograms and adds them to a list
...
void Mg22Matrices()
{
   ...
   TList* list = Mg22Matrices(AnalysisTree, TPPG, TGRSIRunInfo, 0.);
...
TList* Mg22Matrices(TTree* tree, TPPG* ppg, TGRSIRunInfo* runInfo, long maxEntries = 0, TStopwatch* w = nullptr)
{
   ...
   // THnSparseF* gglongmatrixE = new THnSparseF("gglongmatrixE","#gamma-#gamma matrix sorted in
   // E",3,sparsebins,sparselow,sparsehigh); list->Add(gglongmatrixE);
   auto* gglongmatrixT =
      new THnSparseF("gglongmatrixT", "#gamma-#gamma matrix sorted in E", 3, sparsebins, sparselow, sparsehigh);
   list->Add(gglongmatrixT);
   auto* bgTDiffgE =
      new TH2D("bgTDiffgE", "#beta-#gamma Time Difference vs. #gamma E", 2000, 0., 2000., 400, -100., 300.);
   list->Add(bgTDiffgE);
   ...
   auto* gammaSingles = new TH1D("gammaSingles", "#gamma singles;energy[keV]", nofBins * 4, low, high);
   ...
   for(entry = 1; entry < maxEntries; ++entry) { // Only loop over the set number of entries
      ...
      */
      ...
      for(one = 0; one < (int)grif->GetMultiplicity(); ++one) {
         if(grif->GetGriffinHit(one)->GetKValue() != 700) {
            continue;
         }
         ...
         if(grif->GetGriffinHit(one)->GetArrayNumber() < lastTimeStamp.size()) {
            if(lastTimeStamp.at(grif->GetGriffinHit(one)->GetArrayNumber()) > 0) {
               gTimeDiff->Fill(grif->GetHit(one)->GetTime() -
         ...
         for(two = 0; two < (int)grif->GetMultiplicity(); ++two) {
            if(grif->GetGriffinHit(two)->GetKValue() != 700) {
               continue;
            }
            if(two == one) { // If we are looking at the same gamma we don't want to call it a coincidence
               continue;
            }
            Double_t fillval[3] = {grif->GetHit(one)->GetEnergy(), grif->GetHit(two)->GetEnergy(),
                                   10. * (grif->GetHit(one)->GetTimeStamp() - grif->GetHit(two)->GetTimeStamp())};
            gglongmatrixT->Fill(fillval);
            if(grif->GetHit(one)->GetEnergy() == grif->GetHit(two)->GetEnergy()) {
               std::cout<<"Sparse: energies equal"<<std::endl;
            }
            if(grif->GetHit(one)->GetEnergy() >
               grif->GetHit(two)->GetEnergy()) { // These should not be equal, if they are output a warning?
      ...
      if(gotSceptar && scep->GetMultiplicity() > 0) {
         ...
         for(int b = 0; b < scep->GetMultiplicity(); ++b) {
            ...
            if((ppg != nullptr) && !plotted_flag) { // Fill on first hit only.
               betaSinglesCyc->Fill(((ppg->GetTimeInCycle(scep->GetHit(b)->GetTimeStamp())) / 1e5),
                                    ppg->GetCycleNumber(static_cast<ULong64_t>(scep->GetHit(b)->GetTimeStamp())));
         ...
         for(one = 0; one < (int)grif->GetMultiplicity(); ++one) {
            ...
            for(int b = 0; b < scep->GetMultiplicity(); ++b) {
               ...
               bgTDiffgE->Fill(grif->GetHit(one)->GetEnergy(),
                               (grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime()) / 1e10);
                  ...
                  if(!found) {
                     ...
                     if(ppg != nullptr) {
                        gammaSinglesBCyc->Fill(
                           ppg->GetTimeInCycle(static_cast<ULong64_t>(grif->GetHit(one)->GetTimeStamp())) / 1e5,
                           grif->GetGriffinHit(one)->GetEnergy());
                     }
                  }
                  ...
                  if(!found) {
                     ...
                     if(ppg != nullptr) {
                        gammaAddbackBCyc->Fill(
                           ppg->GetTimeInCycle(static_cast<ULong64_t>(grif->GetAddbackHit(one)->GetTimeStamp())) / 1e5,
                           grif->GetAddbackHit(one)->GetEnergy());
                     }
                  }
...
int main(int argc, char** argv)
{
   ...
   w.Continue();
   if(argc < 4) {
      list = Mg22Matrices(tree, myPPG, runInfo, 0, &w);
   } else {
