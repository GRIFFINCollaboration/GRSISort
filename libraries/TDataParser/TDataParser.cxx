			...
			default:
				if((packet & 0x80000000) == 0x00000000) {
					...
					if(x+1 < size &&  (*(data+x+1) & 0x80000000) == 0x0) {
						...
						EventFrag->SetPulseHeight(*(data+x) & 0x03ffffff);
						++x;
						EventFrag->SetIntLength(tmp | (*(data+x) & 0x7c000000) >> 26);
						EventFrag->SetCfd(*(data+x) & 0x03ffffff);
					} else {
