...
class TDetBuildingLoop : public StoppableThread {
	public:
		...
		size_t GetItemsPushed()  { if(fOutputQueues.size() > 0) return fOutputQueues.back()->ItemsPushed(); return std::numeric_limits<size_t>::max(); } //this should work fine as all loops are always filled at the same time
