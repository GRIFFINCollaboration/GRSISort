...
int GValue::ReadValFile(const char* filename, Option_t* opt)
{
   ...
   if(!infile) {
      std::cerr << __PRETTY_FUNCTION__ << ":  could not open infile " << infilename << std::endl;   // NOLINT(cppcoreguidelines-pro-type-const-cast)
      return -2;
   }
   infile.seekg(0, std::ios::end);
   size_t length = infile.tellg();
   if(length == 0) {
      std::cerr << __PRETTY_FUNCTION__ << ":  infile " << infilename << " appears to be empty." << std::endl;   // NOLINT(cppcoreguidelines-pro-type-const-cast)
      return -2;
   }
