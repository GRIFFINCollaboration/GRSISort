...
// g++ LeanMatrices.cxx -std=c++0x -I$GRSISYS/include -L$GRSISYS/libraries -lAnalysisTreeBuilder -lGriffin -lSceptar
...
TList* LeanMatrices(TTree* tree, TPPG* ppg, TGRSIRunInfo* runInfo, long maxEntries = 0, TStopwatch* w = nullptr)
{
   ...
   auto* ggTimeDiff = new TH1D("ggTimeDiff", "#gamma-#gamma time difference", 3000, 0, 3000);
   list->Add(ggTimeDiff);
            ...
            for(int b = 0; b < scep->GetMultiplicity(); ++b) {
               ...
               if((gbTlow <= grif->GetHit(one)->GetTime() - scep->GetHit(b)->GetTime()) &&
                  ...
                  if(!found) {
                     ...
                     if(ppg != nullptr) {
                        // gammaSinglesBCyc->Fill(ppg->GetTimeInCycle((ULong64_t)(grif->GetHit(one)->GetTimeStamp()))/1e5,
                        // grif->GetGriffinHit(one)->GetEnergy());
                        gammaSinglesBCyc->Fill(grif->GetHit(one)->GetCycleTimeStamp() / 1e5,
                                               grif->GetGriffinHit(one)->GetEnergy());
                        // std::cout<<grif->GetHit(one)->GetCycleTimeStamp()<<" =>
                        // "<<grif->GetHit(one)->GetCycleTimeStamp()/1e5<<" = bin
                        // "<<gammaSinglesBCyc->GetXaxis()->FindBin(grif->GetHit(one)->GetCycleTimeStamp()/1e5)<<std::endl;
                     }
                  }
